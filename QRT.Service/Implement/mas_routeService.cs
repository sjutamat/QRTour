...
namespace QRT.Service.Implement
{
    public class mas_routeService:Imas_routeService
    {
        ...
        public List<route_item> GetRouteItem(UserViewModel user)
        {
            ...
            if (user.role != 1)
            {
                admin_id = _viewer.Filter(c => c.user_id == user.id).Select(s => s.admin_id).SingleOrDefault();
            }
            else
            {
                admin_id = user.id;
            }
            var routeData = _route.Filter(c => c.route_active == "A" && c.adminid_create == admin_id).ToList();
            if (routeData != null)
            {
